...
function getGrammarBoundary(lessonNumber: number): string {
  if (lessonNumber <= 35) {
    return 'ТОЛЬКО настоящее время (הוֹוֶה) и простые фразы (יֵשׁ/אֵין, זֶה, שֶׁל). Прошедшее и будущее время СТРОГО ЗАПРЕЩЕНЫ!';
  }
  if (lessonNumber <= 50) {
    return 'Настоящее (הוֹוֶה) и прошедшее время (זְמַן עָבָר). Будущее время ЗАПРЕЩЕНО!';
  }
...
4. Если ученик путает роли (например, говорит «אני משכיר דירה» вместо «שוכר»): доброжелательно поправь его в образе: «רגע, אני בעל הדירה, אני משכיר! אתה מחפש לשכור דירה?».
