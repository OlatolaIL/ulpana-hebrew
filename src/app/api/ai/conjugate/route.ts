...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemPrompt = `Ты — профессиональный лингвистический генератор таблиц спряжения глаголов иврита в стиле Pealim для русскоговорящих студентов ульпана.
Пользователь запросил полное спряжение для глагола или глагольной формы: "${verb}".
Контекст: "${context || verb}".

Твоя задача — составить точную таблицу спряжения глагола во всех временах:
...
3. Корень (שורש): 3-4 буквы через дефис (например: כ-ת-ב).
4. Настоящее время (הווה): 4 формы (זכר יחיד, נקבה יחידה, זכר רבים, נקבה רבות).
5. Прошедшее время (עבר): 8 форм (אני, אתה, את, הוא, היא, אנחנו, אתם/אתן, הם/הן).
6. Будущее время (עתיד): 7 форм (אני, אתה/היא, את, הוא, אנחנו, אתם/אתן, הם/הן).
7. Повелительное наклонение (ציווי): 3 формы (אתה, את, אתם/אתן).

КРИТИЧЕСКИЕ ПРАВИЛА:
- Все формы на иврите ОБЯЗАНЫ иметь точные полные огласовки (נִקּוּד).
- Русская транскрипция ТОЛЬКО на кириллице с ударением (´). Букву ה (хей) СТРОГО передавать как латинскую "h" ("hа-", "hу", "hи", "hолéх"). Буквы ח и כ — как русская "х".
...
Ты ОБЯЗАН ответить СТРОГО валидным JSON-объектом следующего формата:
{
  ...
  "past": [
    ...
    { "pronoun": "אַתֶּם / אַתֶּן (вы)", "hebrew": "כְּתַבְתֶּם / כְּתַבְתֶּן", "cyrillic_transcription": "ктавтéм / ктавтéн", "russian_translation": "вы писали" },
  ...
  "future": [
    ...
    { "pronoun": "אַתָּה / הִיא (ты м.р. / она)", "hebrew": "תִּכְתֹּב", "cyrillic_transcription": "тихтóв", "russian_translation": "ты напишешь / она напишет" },
    ...
    { "pronoun": "אַתֶּם / אַתֶּן (вы)", "hebrew": "תִּכְתְּבוּ", "cyrillic_transcription": "тихтэвӯ", "russian_translation": "вы напишете" },
    { "pronoun": "הֵם / הֵן (они)", "hebrew": "יִכְתְּבוּ", "cyrillic_transcription": "йихтэвӯ", "russian_translation": "они напишут" }
  ],
  "imperative": [
