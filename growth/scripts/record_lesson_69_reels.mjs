...
import { CUES_CONFIG } from './generate_lesson_69_audio.mjs';
...
const SCENE_HTML = path.resolve(ROOT, 'growth/scenes/ze_alay_lesson_69/index.html');
 ...
 */
async function buildLesson69MasterAudio(outputPath = path.join(OUTPUT_DIR, 'reels_lesson_69_audio.wav')) {
  console.log('🎙️ Сборка мастер-аудиодорожки Урока 69 с косинусным de-clicking и расчетом таймкодов...');
  const sampleRate = 44100;
  const numChannels = 2;

  const loadedClips = [];
  for (const cue of CUES_CONFIG) {
    const filename = `l69_${cue.id}.wav`;
  ...
  // 1. Кафе / Введение: 0
  // 2. Паника «Что на мне?!»: старт cue_03_panic
  // 3. Решение Урок 69 + Звонок Надава: старт cue_04_explainer
  // 4. Финальный оффер: старт cue_06_outro
  const sceneTimings = {
    tIntro: 0,
    tPanic: visualTimings['cue_03_panic'],
    tSolution: visualTimings['cue_04_explainer'],
    tOutro: visualTimings['cue_06_outro'],
    totalDurationMs: Math.round(totalDurationSec * 1000),
  };

  console.log('⏱️ Рассчитанные тайминги сцен Урока 69 (ms):', sceneTimings);
  fs.writeFileSync(path.join(OUTPUT_DIR, 'lesson_69_scene_timings.json'), JSON.stringify(sceneTimings, null, 2));
...
async function recordAndRender(sceneTimings, totalDurationSec, masterAudioPath) {
  console.log('\n🎬 Запуск Playwright для покадровой видеозаписи сцены Урока 69...');
  const tempVideoDir = path.join(OUTPUT_DIR, 'temp_l69_record');
  ...
  const finalMp4 = path.join(OUTPUT_DIR, 'reels_lesson_69_ze_alay.mp4');
  ...
  console.log(`\n🎉 РОЛИК УРОКА 69 УСПЕШНО СМОНТИРОВАН!`);
  console.log(`📁 Файл: ${finalMp4} (${(stat.size / 1024 / 1024).toFixed(2)} MB, ${totalDurationSec.toFixed(1)} сек)`);
  return finalMp4;
}

async function main() {
  const { outputPath: masterAudioPath, sceneTimings, totalDurationSec } = await buildLesson69MasterAudio();
